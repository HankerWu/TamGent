...
def save_checkpoint(args, trainer, epoch_itr, val_loss):
    ...
    if len(checkpoints) > 0:
        ...
        write_timer.stop()
        if checkpoint_conds['checkpoint_best.pt']:
            best_str = 'best, '
        else:
            best_str = ''
        print(f'| saved checkpoint {checkpoints[0]} ({best_str}epoch {epoch} @ {updates} updates) '
...
def _upgrade_state_dict(state):
    ...
    try:
        ...
        if state['args'].task in tasks.TASK_REGISTRY:
            # [NOTE]: Sometimes (e.g., initialize from pretrained RoBERTa model), task will not in registry.
            registry.set_defaults(state['args'], tasks.TASK_REGISTRY[state['args'].task])
        if state['args'].arch in models.ARCH_MODEL_REGISTRY:
