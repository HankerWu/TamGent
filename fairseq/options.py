...
def get_lm_extract_features_parser(default_task='language_modeling'):
    parser = get_parser('Extract Language Model Features', default_task)
    add_dataset_args(parser, gen=True)
    add_model_args(parser)
    add_generation_args(parser)
    return parser
...
def comma_assign_dict(x, type=None) -> dict:
    """Create a dictionary from the argument string.

    >>> comma_assign_dict(None)
    {}
    >>> comma_assign_dict('x.txt,y.txt')
    {'0': 'x.txt', '1': 'y.txt'}
    >>> comma_assign_dict('0=x.txt,2=y.txt')
    {'0': 'x.txt', '2': 'y.txt'}
    >>> comma_assign_dict('0=1024,1=1280', type=int)
    {'0': 1024, '1': 1280}
    """
    result = {}
    if x is None:
    ...
    if '=' in x:
        ...
        for i, value in enumerate(items):
            result[str(i)] = _convert(value)
    return result


def check_dict_by_keys(arg, keys: list) -> dict:
    if not isinstance(arg, dict):
        single_value = arg
        return {key: single_value for key in keys}
    if len(arg) == 1 and '0' in arg:
        single_value = arg['0']
        return {key: single_value for key in keys}
    if arg.keys() != set(keys):
        raise RuntimeError('data ID mismatch between argument and keys')
    return arg
...
def parse_args_and_arch(parser, input_args=None, parse_known=False, suppress_defaults=False):
    if suppress_defaults:
        ...
        suppressed_parser = argparse.ArgumentParser(add_help=False, parents=[parser], fromfile_prefix_chars='@')
...
def add_generation_args(parser):
    ...
    group.add_argument('--codegen-pov-replace-unk', action='store_true', default=True,
                       help='Replace unknown token to "MASK" in codegen PoV computation')
    group.add_argument('--no-codegen-pov-replace-unk', dest='codegen_pov_replace_unk', action='store_false', default=True,
                       help='Disable replace unknown token to "MASK" in codegen PoV computation')
