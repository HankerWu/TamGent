...
class tensorboard_log_wrapper(progress_bar):
    ...
    _UNIQUE_NAME_THIS_RUN = None    # Keep same in a single run.

    def __init__(self, wrapped_bar, tensorboard_logdir, args):
        self.wrapped_bar = wrapped_bar
        self.tensorboard_logdir = tensorboard_logdir
        self.args = args
        if self._UNIQUE_NAME_THIS_RUN is None:
            type(self)._UNIQUE_NAME_THIS_RUN = self._unique_name()
    ...
    def _unique_name() -> str:
        ...
        return datetime.now().strftime('%Y-%m-%d_%H-%M-%S') + '_' + socket.gethostname()
    ...
    def _writer(self, key):
        ...
        if key not in self._writers:
            self._writers[key] = self.SummaryWriter(
                os.path.join(self.tensorboard_logdir, self._UNIQUE_NAME_THIS_RUN, key),
            )
