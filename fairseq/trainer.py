...
class Trainer(object):
    ...
    def __init__(self, args, task, model, criterion, dummy_batch=None, oom_batch=None):
        ...
        # [NOTE]: Special processing for L2T-V2 models.
        if hasattr(model, 'L2TV2_TAG') and args.enable_teacher:
            self._running_l2tv2 = True

            # Get the student backup model for hyper-gradient.
            self.l2tv2_student_backup = model.build_model(args, task, build_teacher=False)
            self.l2tv2_batch_data = []

            # Get the repeated valid dataset iterator to compute the teacher metric
            self._l2tv2_batch_itr = task.get_batch_iterator(
            ...
            def _repeater(batch_itr):
                while True:
                    yield from batch_itr.next_epoch_itr(shuffle=True)

            self.l2tv2_repeated_valid_itr = _repeater(self._l2tv2_batch_itr)
        ...
        if self.cuda:
            ...
            if getattr(self, '_running_l2tv2', False) is True:
                self.l2tv2_student_backup = self.l2tv2_student_backup.cuda()
    ...
    def _build_optimizer(self):
        if getattr(self, '_running_l2tv2', False) is True:
            model_parameters_itr = self.model.student_parameters()
        else:
            model_parameters_itr = self.model.parameters()
        params = list(filter(lambda p: p.requires_grad, model_parameters_itr))
        if self.args.fp16:
    ...
    def load_checkpoint(
        ...
        if os.path.exists(filename):
            ...
            if getattr(self.args, 'warm_start_student', None) is not None:
                print('| loading the checkpoint as a warm start student')
                model_state_dict = self.get_model().state_dict()
                keys = list(model_state_dict.keys())
                for key in keys:
                    if key.startswith('teacher.'):
                        state['model'][key] = model_state_dict[key]
                state_dict_keys = list(state['model'].keys())
    ...
    def train_step(self, samples, dummy_batch=False, raise_oom=False):
        ...
        for i, sample in enumerate(samples):
            ...
            # [NOTE]: OOM in hyper-gradient step usually means training failed, so we do not catch OOM error here.
            if getattr(self, '_running_l2tv2', False) is True:
                self.model.maybe_hyper_gradient_step(self, sample)
