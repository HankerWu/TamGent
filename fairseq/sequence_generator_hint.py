...
class SequenceGeneratorWithHint(object):
    def __init__(
        self,
        tgt_dict,
        beam_size=1,
    ...
    def generate(
        ...
        **kwargs
        ...
        tgt_lengths = (prev_output_tokens.ne(self.eos) & prev_output_tokens.ne(self.pad)).long().sum(dim=1)
        ...
        if self.hint:
            max_hint_len = math.floor(torch.max(tgt_lengths) * self.hint_rate)
            prefix_tokens = src_tokens.new(bsz, max_hint_len).long().fill_(self.pad)
            
            for i in range(bsz):
                hint_len = math.floor(tgt_lengths[i] * self.hint_rate)
                prefix_tokens[i, :hint_len] = prev_output_tokens[i, :hint_len] if not self.left_pad_target else prev_output_tokens[i, -hint_len:]
