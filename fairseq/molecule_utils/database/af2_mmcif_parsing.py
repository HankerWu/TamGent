...
class AF2MmcifObject:
    ...
    Contains:
      ...
      mmcif_to_author_chain_id: maps internal mmCIF chain ids to chain ids used
        the authors / Biopython. e.g. {'A': 'A', 'B': 'A'}
      raw_string: The raw string used to construct the AF2MmcifObject.
    """
    file_id: str
    header: PdbHeader
    structure: PdbModel
...
class ParsingResult:
    ...
    Contains:
      mmcif_object: A AF2MmcifObject, may be None if no chain could be successfully
        parsed.
      errors: A dict mapping (file_id, chain_id) to any exception generated.
    """
    mmcif_object: Optional[AF2MmcifObject]
...
def mmcif_loop_to_dict(prefix: str,
                       ...
                       ) -> Mapping[Union[str, Tuple[str]], Mapping[str, str]]:
    ...
    if isinstance(index, str):
        ...
        return {tuple(entry[i] for i in index): entry for entry in entries}


def parse_string(*, file_id: str, mmcif_string: str, catch_all_errors: bool = True) -> ParsingResult:
    """Entry point, parses an mmcif_string."""
    handle = io.StringIO(mmcif_string)
    return parse(file_id=file_id, mmcif_handle=handle, catch_all_errors=catch_all_errors)


def parse(*, file_id: str, mmcif_handle: io.TextIOBase, catch_all_errors: bool = True) -> ParsingResult:
    """Entry point, parses an mmcif_handle.

    Args:
      file_id: A string identifier for this file. Should be unique within the
        collection of files being processed.
      mmcif_handle: Handle an mmCIF file.
    ...
    try:
        ...
        full_structure = parser.get_structure('', mmcif_handle)
        ...
        valid_chains = get_protein_chains(parsed_info=parsed_info)
        if not valid_chains:
        ...
        if not catch_all_errors:
            raise AF2MmCIFParseError(f'Failed to parse mmCIF file {file_id}.') from e
